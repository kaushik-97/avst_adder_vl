...
class avst_out_driver: uvm_component
{
 ...
 // avst_item_out delay_out_cov;
  ...
  override void run_phase(uvm_phase phase) {
   ...
   // to_snooper_out.write(delay_out_cov);
  }
...
class avst_coverage: uvm_subscriber!(avst_item)
{
  ...
  @UVM_BUILD {
    ...
    uvm_analysis_imp!(avst_coverage.write) frm_monitor_in;
    //uvm_analysis_imp!(avst_coverage.write_out) frm_monitor_out;
  }
  ...
  override void write(avst_item a) {
    ...
    uvm_info("avst_coverage",format("delay at cov is #%d", a.delay),UVM_DEBUG);
    // Pass the coverage data to the sample function
    test_cg_in.sample(a.delay, a.data, a.end);
  }

  // override void write_out(avst_item_out b) {
  //   uvm_info("avst_coverage", "Received data in coverage class", UVM_DEBUG);
  //   uvm_info("avst_coverage",format("delay at cov is #%d", b.delay),UVM_DEBUG);
  //   // Pass the coverage data to the sample function
  //   test_cg.sample(b.delay);
  // }
...
class avst_agent: uvm_agent
{
  ...
  override void connect_phase(uvm_phase phase) {
    ...
    req_snooper.covport.connect(coverage.frm_monitor_in);
    //rsp_snooper.covport.connect(coverage.frm_monitor_out);
